...
def fix_truncation(content):
    """Fix file truncated by token limit - find last complete JSX closing and close properly"""
    ...
    log("  Файл обрезан токенами — восстанавливаю закрытие")
    
    # Find the last complete closing tag
    # Look for last </section>, </div>, </p>, </ul>, </ol>
    closing_tags = ["</section>", "</div>", "</p>", "</ul>", "</ol>", "</li>"]
    last_pos = -1
    ...
    if last_pos > 0:
        ...
        # Now close the component properly
        content += "\n        </article>\n      </div>\n    </div>\n  )\n}"
...
def inject_products(content, products):
    """Inject products block before </article>"""
...
def validate_tsx(content, slug):
    ...
    if not (stripped.endswith("}") or stripped.endswith(">")):
        log("  FAIL: файл обрезан (не заканчивается на } или >)")
        ok = False
...
def generate_blog_article(topic):
    ...
    prompt = (
        ...
        "        [СТАТЬЯ: 5 секций h2, каждая 150-200 слов, про {title}]\n"
        ...
        "- НЕ добавляй блок товаров — я добавлю его сам\n"
        "- Закончи файл точно на символе }} — не обрывай на полуслове\n"
