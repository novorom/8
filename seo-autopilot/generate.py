...
def main():
    ...
    try:
        ...
        # Inject products block if missing (Haiku sometimes skips it)
        if "Товары из этой статьи" not in tsx_content:
            log(f"  ⚠️ Блок товаров не найден — вставляю принудительно")
            products = next_topic["products"]
            products_block = f"""
          <div className="mt-8 p-5 rounded-xl bg-muted/50 border border-border">
            ...
            # Insert before closing </div></div> at end
            tsx_content = tsx_content.rstrip()
            if tsx_content.endswith("}"):
                # Find last </div> before the closing function brace
                insert_pos = tsx_content.rfind("    </div>
  )
}")
                if insert_pos != -1:
                    tsx_content = tsx_content[:insert_pos] + products_block + "
    </div>
  )
}"
            log(f"  ✅ Блок товаров вставлен принудительно")
