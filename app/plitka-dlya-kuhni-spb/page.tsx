...
export const metadata: Metadata = {
  ...
  description: "Керамическая плитка и керамогранит для кухни Cersanit в СПб. Влагостойкая, жиростойкая, легко моется. склад в СПб, доставка по СПб от 1 дня. Цены от 472 руб/м2.",
...
const faq = [
  ...
  { question: "Как быстро доставите в СПб?", answer: "Доставка по СПб и ЛО — 1-2 рабочих дня. Самовывоз со склада в СПб бесплатный в день оплаты. Пн-Пт 10:00-16:45." },
]
...
export default function PlitkaKuhnya() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-primary text-primary-foreground py-12 lg:py-16"><div className="mx-auto max-w-7xl px-4"><h1 className="text-3xl lg:text-4xl xl:text-5xl font-bold text-balance">Плитка для кухни Cersanit в Санкт-Петербурге</h1><p className="mt-4 text-primary-foreground/80 text-lg leading-relaxed max-w-3xl">Влагостойкая, жиростойкая, легко моется — для пола и стен кухни. {items.length} позиций в наличии на складе в СПб.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="#products" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-background text-foreground font-medium text-sm hover:bg-background/90 transition-colors">Смотреть товары <ChevronRight className="h-4 w-4" /></Link><a href={`tel:${PHONE_RAW}`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-primary-foreground/30 text-primary-foreground font-medium text-sm hover:bg-primary-foreground/10 transition-colors"><Phone className="h-4 w-4" /> {PHONE}</a></div></div></section>
      <section id="products" className="py-12 lg:py-16"><div className="mx-auto max-w-7xl px-4"><h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">Плитка для кухни — {items.length} позиций</h2><p className="text-muted-foreground mb-8">Все товары в наличии на складе в СПб</p><div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">{items.map((p,i) => <ProductCard key={p.id} product={p} priority={i<4} />)}</div></div></section>
      <section className="py-12 lg:py-16 bg-muted/30"><div className="mx-auto max-w-4xl px-4 flex flex-col gap-8">
        ...
        <div><h2 className="text-xl lg:text-2xl font-bold text-foreground mb-4">Купить плитку для кухни в СПб</h2><p className="text-foreground/80 leading-relaxed">Склад в СПб (15-20 мин от КАД). Самовывоз бесплатный. Доставка по СПб и ЛО от 1-2 рабочих дней. Бесплатный расчёт количества плитки.</p></div>
      </div></section>
